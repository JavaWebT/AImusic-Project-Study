...
### Native支付流程
- 1. 用户前端选微信支付，商户向微信请求支付二维码地址，得到后转成二维码展示。用户扫码支付成功，微信向商户回调。回调含序列号、签名值等 4 个验签信息，商户据此构造验签串，用微信平台证书或公钥验签。通过后，用商户平台 APIv3 密钥等对回调业务信息解密，获取交易号、金额等内容 。
- 2. 更新状态带上老状态。更新积分在数据库里操作，不要先查后改，不然会有并发的问题
     1. 在处理并发更新业务时，可根据操作返回值判断数据是否已被修改，若返回值表明未成功更新（如更新行数为 0），则认为数据已被其他线程处理，当前线程不再重复操作，以此避免并发带来的重复更新等问题 。(例如：在payOrderSuccess方法中，有两个线程可能同时调用它，通过检查订单状态判断是否已支付，未支付则更新订单信息。更新操作返回updateCount，若为 0，说明订单可能已被其他线程更新，当前线程不再重复处理，以此来应对并发更新问题 。)
    
### P27超时订单
- 1. 用户token:当用户登录，应该将缓存中的token信息续期；
  2. appConfig与application.ylm文件时对应的，注意要自己手写，保证这两个能够链接上，
