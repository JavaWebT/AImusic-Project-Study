...
- 用户支付之后，微信会给出回调结果通知
- 微信支付的支付产品主要包含以下几种
- 1. JSAPI：在微信客户端浏览器中。下单、支付页面、支付方式、返回到浏览器页面、查看订单
  2. APP支付：再商户自己APP中。下单、支付页面、支付方式、返回到商户APP、查看订单
  4. 小程序支付：再商户小程序中。下单、支付页面、支付方式、返回到商户小程序、查看订单
  5. NATIVE支付：在PC端浏览器中。返回给用户商户的二维码、扫码（不支持长按，或者扫描相册）、支付、查看订单
  6. H5支付：在手机浏览器中。下单、支付页面、支付方式、返回到浏览器页面、查看订单
