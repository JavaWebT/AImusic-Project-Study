...
- 2. 在处理并发更新业务时，可根据操作返回值判断数据是否已被修改，若返回值表明未成功更新（如更新行数为 0），则认为数据已被其他线程处理，当前线程不再重复操作，以此避免并发带来的重复更新等问题 。(例如：在payOrderSuccess方法中，通过检查订单状态判断是否已支付，未支付则更新订单信息。更新操作返回updateCount，若为 0，说明订单可能已被其他线程更新，当前线程不再重复处理，以此来应对并发更新问题 。)
